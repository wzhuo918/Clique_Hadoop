...
public class RunOver {
	...
	public void doStep1(String[] args) throws Exception {
		...
		Job job = new Job(conf, "binary step0");
		...
		FileOutputFormat.setOutputPath(job, new Path("Cliqueout_Binary/"+args[args.length -2] +"_"+ pre));
	...
	public void doStep2(String[] args) throws IOException,
		...
		String in = "/user/youli/CliqueTemData/binaryinputR/";

		Job job = new Job(conf, "binary step"+pre);
			...
			while (thisphasesize != 0) {
				...
				Process p = Runtime
						...
						.exec(new String[] { "/bin/sh", "-c",
								hadoophome+"bin/hadoop fs -rmr /user/youli/CliqueTemData/binaryinputR/" });
				p.waitFor();
				p.destroy();
				RemoteSSH.batch();
				this.wait(15000); 
